...
- v_rentals is the result of a query that gets the count of rentals where inventory_id matches with the one passed in parameters. 
    If is equal to 0 (there is no rentals), it will throw TRUE (inventory has stock).
- v_out is the result of a query that also gets the count of rental_id from the table inventory, matching the inventory_id with the one passed in parameters,
    but with the filter that return_date is null (film not returned). If v_out is greater than 0, returns FALSE, otherwise, TRUE.
*/
